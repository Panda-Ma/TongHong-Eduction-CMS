...
import {getClassStuNum, getNum, getPercentage} from "/@/api/home";
...
export default defineComponent({
  ...
  setup: function () {
    ...
    // 柱状图
    const initBarChart = () => {
      getClassStuNum().then(res=>{
        let className=[]
        let studentNum=[]
        for(let i=0;i<res.data.length;i++){
          className.push(res.data[i].className)
          studentNum.push(res.data[i].studentNum)
        }
    ...
    const initPieChart = () => {
      getPercentage().then((res: any) => {
          ...
          let getname = Object.keys(res.map)
          let getvalue = Object.values(res.map)
          let data = [];
    ...
    onMounted(() => {
      getNum().then((res: any) => {
        state.homeOne[0].num1 = res.map.courseNum
        state.homeOne[0].num3 = '课程数'
        state.homeOne[1].num1 = res.map.classNum
        state.homeOne[1].num3 = '班级数'
        state.homeOne[2].num1 = res.map.teacherNum
        state.homeOne[2].num3 = '教师人数'
        state.homeOne[3].num1 = res.map.studentNum
        state.homeOne[3].num3 = '学生人数'
      })
      initEchartsResize();
    });
    // 由于页面缓存原因，keep-alive
